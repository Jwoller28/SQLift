...
export function GroupProvider({ children }: { children: React.ReactNode }) {
  ...
  async function fetchGroups() {
    try {
      ...
      const res = await fetch(`http://3.142.210.41:8081/groups?userId=${id}`, {
        headers: { 
  ...
  async function createGroup(name: string) {
    try {
      const token = localStorage.getItem('token') ? JSON.parse(String(localStorage.getItem('token'))) : null;
      const res = await fetch('http://3.142.210.41:8081/groups', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  async function joinGroup(groupId: number) {
    try {
      const token = localStorage.getItem('token') ? JSON.parse(String(localStorage.getItem('token'))) : null;
      const id = localStorage.getItem('id') ? JSON.parse(String(localStorage.getItem('id'))) : null;
      const res = await fetch(`http://3.142.210.41:8081/groups/${groupId}/join?userId=${id}`, {
  ...
  async function leaveGroup(groupId: number) {
    try {
      const token = localStorage.getItem('token') ? JSON.parse(String(localStorage.getItem('token'))) : null;
      const id = localStorage.getItem('id') ? JSON.parse(String(localStorage.getItem('id'))) : null;
      const res = await fetch(`http://3.142.210.41:8081/groups/${groupId}/leave?userId=${id}`, {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  async function fetchGroupEvents(groupId: number): Promise<GroupEvent[]> {
    try {
      const token = localStorage.getItem('token') ? JSON.parse(String(localStorage.getItem('token'))) : null;
      const res = await fetch(`http://3.142.210.41:8081/groups/${groupId}/events`, {
        headers: { 
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
  ...
  async function createGroupEvent(
    ...
    try {
      ...
      const endpoint = isPersonalEvent
        ? `http://3.142.210.41:8081/personal-events`
        : `http://3.142.210.41:8081/groups/${groupId}/events`;
