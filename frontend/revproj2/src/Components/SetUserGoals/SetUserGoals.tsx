...
function SetUserGoals() {
  ...
  useEffect(() => {
    ...
    const userValidToken = async () => {
      if (!token) return;
      try {
        const responseValidToken = await fetch('http://18.221.145.66:8080/me', {
          headers: {
  ...
  useEffect(() => {
    ...
    const getUserId = async () => {
      try {
        const userIdResponse = await fetch(`http://18.221.145.66:8080/username/${username}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  async function handleGoalSubmit(event: FormEvent) {
    ...
    try {
      const goalResponse = await fetch('http://18.221.145.66:8080/goal', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
