...
export const sendPost = async (formData : FormData) => {
	...
	const cleanToken = Token?.replace(/"/g, "");
        const url = "http://3.142.210.41:8081/posts";
...
export const getPostPhoto = async (fileName : string) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
        const url = `http://3.142.210.41:8081/s3bucket/trackr-photo-store/download/${fileName}`;
...
export const sendPostPhoto = async (photo : FormData) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
        const url = "http://3.142.210.41:8081/s3bucket/trackr-photo-store/upload";
...
export const getPost = async () => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
        const url = "http://3.142.210.41:8081/live/posts";
...
export const usernameifAuthorized = async () => {
	...
	const cleanToken = Token?.replace(/"/g, "");
	console.log(Token);
	const url = "http://3.142.210.41:8081/me";
...
export const getTrackers = async (userId: number, goalId: number) => {
	try {
	 ...
	 const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
	 const url =`http://3.142.210.41:8081/Tracker/${userId}/${goalId}`;
...
export const getUserByUsername = async (username : string) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
	const url1= `http://3.142.210.41:8081/username/${username}`;
...
export const getGoalbyUserId = async(userId : number) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
	const url2 = `http://3.142.210.41:8081/goalUser/${userId}`;
...
export const getGoalsbyUserId = async(userId : number) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
	const url2 = `http://3.142.210.41:8081/goalsUser/${userId}`;
...
export const getCommentsByPost = async(postId : number) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
	const url = `http://3.142.210.41:8081/fetch/comment/${postId}`;
...
export const sendComment = async (comment : FormData) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
        const url = "http://3.142.210.41:8081/create/comment";
		console.log(comment);
...
export const sendTypeFilter = async (searchType : searchType) => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
        const url = "http://3.142.210.41:8081/filter";
...
export const getFilteredPost = async () => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
        const url = "http://3.142.210.41:8081/filter/live/post";
...
export const getFilteredStoredPosts = async () => {
	...
	const Token = localStorage.getItem('token');
	const cleanToken = Token?.replace(/"/g, "");
	const url = "http://3.142.210.41:8081/filter/posts";
