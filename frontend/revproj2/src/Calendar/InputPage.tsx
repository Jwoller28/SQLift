...
function InputPage() {
  ...
  useEffect(() => {
    ...
    const fetchMe = async () => {
      try {
        const meRes = await fetch('http://18.116.10.154:8081/me', {
          headers: {
  ...
  useEffect(() => {
    ...
    const fetchUsername = async () => {
      try {
        const userRes = await fetch(`http://18.116.10.154:8081/username/${userName}`, {
  ...
  useEffect(() => {
    ...
    const fetchGoal = async () => {
      try {
        const goalRes = await fetch(`http://18.116.10.154:8081/goalUser/${userId}`, {
          headers: { Authorization: `Bearer ${userToken}` },
        });
        if (!goalRes.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('http://18.116.10.154:8081/Tracker', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${userToken}`,
